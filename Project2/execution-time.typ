...
#let load-scenario(algorithm, scenario, sizes) = {
  sizes.map(n => {
    let path = "results/" + scenario + "/" + algorithm + "/" + str(n) + ".csv"
  ...
  figure(
    cetz.canvas({
      plot.plot(size: (14, 12),
                ...
                x-format: x => $10^#int(x)$,
                y-format: x => $10^#int(x)$,
                x-label: [$log_10(n)$ - Ilość elementów],
                y-label: [$log_10(t)$ - Czas wykonania (#(sym.mu)s)],
      {
        ...
        plot.add(
          d.descending.map(row => (row.at(0), calc.log(row.at(1), base: 10))),
          mark: "square",
          mark-style: (stroke: orange, fill: orange.lighten(40%)),
          style: (stroke: orange.darken(15%) + 2pt),
          label: "Posortowane malejąco"
        )

        plot.add(
          d.ascending.map(row => (row.at(0), calc.log(row.at(1), base: 10))),
          mark: "square",
          mark-style: (stroke: red, fill: red.lighten(40%)),
          style: (stroke: red.darken(15%) + 2pt),
          label: "Posortowane rosnąco"
        )

        plot.add(
          d.adjacent-swaps.map(row => (row.at(0), calc.log(row.at(1), base: 10))),
          mark: "triangle",
          mark-style: (stroke: blue, fill: blue.lighten(40%)),
          style: (stroke: blue.darken(15%) + 2pt),
          label: stack(
            spacing: 0em,
            [Prawie posortowane (sąsiednia wymiana)],
            [],
          ),
        )

        plot.add(
          d.random-swaps.map(row => (row.at(0), calc.log(row.at(1), base: 10))),
          mark: "triangle",
          ...
          label: stack(
            spacing: 0em,
            [Prawie posortowane (globalna wymiana)],
            [],
          ),
        )
